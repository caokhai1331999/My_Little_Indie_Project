...
void loadShader(Shader* shader, char* path){
    ...
    char* Error;
    if(shader->shader_file = CreateFileA(
                                     "basic_shader.vs",
                                     GENERIC_READ,
                                     0,0,
                                     OPEN_EXISTING,
                                     0,0);)
    {
        ...
        shader->file_size = GetFileSize(shader->shaderFile, shader->file_size);
        shader->file_size = safetruncateUint64(shader->file_size);

        if((ReadFile(shader->shader_file, shader->SourcePath, shader->file_size, ByteRead, NULL)) && (shader->file_size == ByteRead)){
